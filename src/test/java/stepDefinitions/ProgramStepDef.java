...
public class ProgramStepDef extends GlobalTestData {
	...
	public void admin_verifies_the_response_payload_with_expected_output_from_the_data_sheet() {
		...
		if (expectedStatus == 201) {
			...
			response.then().log().all().assertThat()
					.body(matchesJsonSchemaInClasspath("schemas/Program/CreateProgramResponseSchema.json"));
	...
	public void admin_sends_get_request_to_get_program_with_different_payload_for_from_data_sheet(
			String scenarioNameFromFeature) throws IOException {
	...
	public void admin_verifies_the_response_payload_with_expected_output_from_the_data_sheet_for_get_program() {
		...
		if (expectedStatus == 200) {
			response.then().assertThat()
					.body(matchesJsonSchemaInClasspath("schemas/Program/GetProgramByProgramIdResponseSchema.json"));
	...
	public void admin_sends_get_request_to_get_all_programs_with_different_payload_for_from_data_sheet(
			String scenarioNameFromFeature) throws IOException {
	...
	public void admin_verifies_the_response_payload_with_expected_output_from_the_data_sheet_for_update_program_by_program_id() {
		...
		if (expectedStatus == 200) {
			...
			response.then().body("programId",
					Matchers.equalTo(GlobalTestData.programIdList.get(programIdList.size() - 1)));
			Assert.assertEquals(actualResponse.getProgramName(), programInput.getProgramName(),
	...
	public void admin_sends_put_request_to_update_program_by_name_with_payload_for_using_data_sheet(String scenario)
			throws IOException {
		String scenarioName = scenario;
		data = ExcelReader.readExcelData("Program", scenarioName);
		if (data != null) {
			...
			if (scenarioName.equalsIgnoreCase(dataSheetTestname)) {
				...
				response = requestSpec.when().log().all().request(httpMethod, endPoint).then().log().all().extract()
						.response();
			}
		}
	}
	...
	public void admin_verifies_the_response_payload_with_expected_output_from_the_data_sheet_for_update_program_by_program_name() {
		...
		if (expectedStatus == 200) {
			ProgramResponse actualResponse = response.as(ProgramResponse.class);
			response.then().assertThat()
					.body(matchesJsonSchemaInClasspath("schemas/Program/UpdateProgramByProgramName.json"));
			...
			Assert.assertEquals(actualResponse.getProgramDescription(), programInput.getProgramDescription(),
					"Program Description is not matching after update.");
			Assert.assertEquals(actualResponse.getProgramStatus(), programInput.getprogramStatus(),
					"Program Status is not matching after update.");
			Assert.assertNotNull(actualResponse.getCreationTime(), "Creation Time should not be null after update.");
			Assert.assertNotNull(actualResponse.getLastModTime(),
	...
	public void admin_sends_delete_request_to_delete_program_by_name_with_payload_for_using_data_sheet(String scenario)
			throws IOException {
		String scenarioName = scenario;
		data = ExcelReader.readExcelData("Program", scenarioName);
		if (data != null) {
	...
	public void admin_verifies_the_response_payload_with_expected_output_from_the_data_sheet_for_delete_program_by_program_name() {
		...
		if (expectedStatus == 200) {
			...
			Assert.assertEquals(response.jsonPath().getString("programStatus"), "Inactive",
					"Program status should be Inactive after deletion");
	...
	public void admin_sends_delete_request_to_delete_program_by_id_with_payload_for_using_data_sheet(String scenario)
			throws IOException {
		String scenarioName = scenario;
		data = ExcelReader.readExcelData("Program", scenarioName);
		if (data != null) {
	...
	public void admin_verifies_the_response_payload_with_expected_output_from_the_data_sheet_for_delete_program_by_program_id() {
		...
		if (expectedStatus == 200) {
			...
			Assert.assertEquals(expectedMessage, actualMessage,
					"Actual and expected message message in output is not as expected");
			Assert.assertEquals(actualSuccessMessage, "success", "Status in output is not as expected");
	...
	public void admin_send_http_method_to_access_endpoint_for_testcase_without_authorization_token(String scenario)
			throws IOException {
		String scenarioName = scenario;
		data = ExcelReader.readExcelData("Program", scenarioName);
		if (data != null) {
	...
	public void admin_send_http_method_to_access_endpoint_for_testcase_with_invalid_authorization_token(
	...
	public void admin_sends_delete_request_to_delete_all_programs() {
		for (int i = programIdList.size() - 1; i >= 0; i--) {
			given().spec(RequestSpecUtil.getRequestSpec()).pathParam("id", programIdList.get(i)).when().log().all()
					.delete("/deletebyprogid/{programId}").then().log().all().assertThat().statusCode(200);
