...
public class ProgramBatchStepDef extends GlobalTestData {
	...
	public void admin_create_post_request_with_valid_data_for_from_excel_sheet(String scenario) throws IOException {
		...
		requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).basePath(data.get("Endpoint")).body(batchData);
	...
	public void admin_create_post_request_with_invalid_input_for_from_excel_sheet(String scenario) throws IOException {
		...
		if (scenario.contains("NoAuth")) {
			requestSpec = given().spec(RequestSpecUtil.getRequestSpecWithoutAuth()).basePath(data.get("Endpoint"))
					.body(data.get("Body"));
		} else {
			requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).basePath(data.get("Endpoint"))
					.body(data.get("Body"));
		}
	}
	...
	public void admin_create_get_request_with_valid_batch_id() throws IOException {
		...
		requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchId", batchId)
	...
	public void admin_create_get_request_by_batch_id_with_invalid_input_for_from_excel_sheet(String scenario)
		...
		data = ExcelReader.readExcelData("Batch", scenario);
		if (scenario.contains("NoAuth")) {
			requestSpec = given().spec(RequestSpecUtil.getRequestSpecWithoutAuth()).pathParam("batchId", batchId)
					.basePath(data.get("Endpoint"));
	...
	public void admin_create_get_request_to_retrieve_batch_with_valid_batch_name() throws IOException {
		...
		requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchName", batchName)
				.basePath(data.get("Endpoint"));
	}
	...
	public void admin_create_get_request_by_batch_name_with_invalid_input_for_from_excel_sheet(String scenario)
			throws IOException {
		data = ExcelReader.readExcelData("Batch", scenario);
		if (scenario.contains("NoAuth")) {
			requestSpec = given().spec(RequestSpecUtil.getRequestSpecWithoutAuth()).pathParam("batchName", batchName)
					.basePath(data.get("Endpoint"));
	...
	public void admin_create_get_request_to_retrieve_batch_with_valid_program_id() throws IOException {
		...
		requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("programId", programId)
				.basePath(data.get("Endpoint"));
	}
	...
	public void admin_create_get_request_by_program_id_with_invalid_input_for_scenario_from_excel_sheet(String scenario)
			throws IOException {
		data = ExcelReader.readExcelData("Batch", scenario);
		if (scenario.contains("NoAuth")) {
			requestSpec = given().spec(RequestSpecUtil.getRequestSpecWithoutAuth()).pathParam("programId", programId)
					.basePath(data.get("Endpoint"));
	...
	public void admin_create_put_request_to_update_batch_with_valid_batch_id_for_scenario(String scenario)
			throws IOException {
		data = ExcelReader.readExcelData("Batch", scenario);
		ObjectMapper mapper = new ObjectMapper();
		...
		requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchId", batchId)
				.basePath(data.get("Endpoint")).body(batchData);
		scenarioContext.setContext("BATCH_NAME", batchData.getBatchName());
	...
	public void admin_create_put_request_with_invalid_input_for_each_from_excel_sheet(String scenario)
			throws IOException {
		data = ExcelReader.readExcelData("Batch", scenario);
		if (scenario.contains("Invalid_BatchId")) {
			...
			requestSpec = given().spec(RequestSpecUtil.getRequestSpecWithoutAuth()).pathParam("batchId", batchId)
					.basePath(data.get("Endpoint")).body(data.get("Body"));
		} else {
			requestSpec = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchId", batchId)
					.basePath(data.get("Endpoint")).body(data.get("Body"));
		}
	...
	public void admin_sends_get_request_to_retrieve_deleted_batch_with_id() throws IOException {
		...
		response = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchId", batchId)
				.basePath(data.get("Endpoint")).when().log().all().get();
	}
	...
	public void admin_sends_get_request_to_retrieve_deleted_batch_with_name() throws IOException {
		...
		response = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchName", batchName)
				.basePath(data.get("Endpoint")).when().log().all().get();
	}
	...
	public void admin_sends_put_request_to_update_deleted_batch_status() throws IOException {
		...
		batchData.setProgramName(programName);
		response = given().spec(RequestSpecUtil.getRequestSpec()).pathParam("batchId", batchId)
				.basePath(data.get("Endpoint")).body(batchData).when().log().all().put();
	}
	...
	public void admin_sends_delete_request_to_delete_all_batches_created() {
		for (int i = batchIds.size() - 1; i >= 0; i--) {
			given().spec(RequestSpecUtil.getRequestSpec()).pathParam("id", batchIds.get(i)).when().log().all()
					.delete("/batches/{id}").then().log().all().assertThat().statusCode(200);
