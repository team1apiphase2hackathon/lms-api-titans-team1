...
public class SkillMasterStepDef extends GlobalTestData {
	...
	public void admin_sends_https_post_request_and_request_body_with_mandatory() throws Exception {
		...
		// Skill name and Skill ID storage
	...
	public void admin_receives_created_status_with_response_body(Integer num) {
		// POST - status code, response body and schema validation
		response.then().log().ifValidationFails().spec(ResponseSpecUtil.status(num))
				.body("skillName", equalTo(skillName))
				.body(matchesJsonSchemaInClasspath("schemas/SkillMaster/CreateSkillResponseSchema.json"));
	}
	...
	public void admin_receives_status_with_response_body(Integer num) {
		response.then().log().ifValidationFails().spec(ResponseSpecUtil.status(num))
				.body(matchesJsonSchemaInClasspath("schemas/SkillMaster/GetAllSkillResponseSchema.json"));
	}
	...
	public void admin_receives_status_with_response_body_for_that_skill(Integer num) {
		response.then().log().ifValidationFails().spec(ResponseSpecUtil.status(num))
				.body(matchesJsonSchemaInClasspath("schemas/SkillMaster/GetSkillBySkillNameResponseSchema.json"));
	}
	...
	public void admin_receives_not_found_status_with_error_message_as_success_as(Integer code, String successParam)
			throws Exception {
		data = ExcelReader.readExcelData("Skill", "GetSkill_InValid");
		...
		response.then().log().ifValidationFails().spec(ResponseSpecUtil.status(code))
				.body("message", equalTo(expectedMessage)).body("success", equalTo(expectedSuccess));
	}
	...
	public void admin_receives_status_with_updated_response_body(Integer num) {
		response.then().log().ifValidationFails().spec(ResponseSpecUtil.status(num)).body("skillName",
				equalTo(updatedskillName));
	}
